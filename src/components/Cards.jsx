...
const Cards = (props) => {
  return (
    <div
      ...
      className={` ${props.width} shrink-0 mb-3  duration-500 cursor-pointer`}
    >
      ...
      <div className="mt-2 font-bold text-xl">{props.title}</div>
      <div>
        <Star className="inline mr-2" />
        <span className="font-bold">{props.rating}</span>
        <span className="font-semibold tracking-tighter">
          <LuDot className="inline font-bold" />
          {props.minTime}-{props.maxTime} mins
        </span>
      </div>
      <div className="text-[#02060ca6] font-semibold text-[14px]">
        {props.name}
      </div>
      <div className="text-[#02060ca6] font-semibold text-[14px]">
        {props.place}
